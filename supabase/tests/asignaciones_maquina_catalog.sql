...
--
-- TEST 18: Finish project with 3 active assignments (2 direct + 1 subproject)
-- Issue #65 — verify finish_proyecto closes all scoped assignments.
--
\echo 'Test 18: finish_proyecto closes 3 assignments (2 direct + 1 subproject)'
savepoint test18;
set local role service_role;

-- Grant projects:finish to admin role
insert into role_capabilities (role_id, capability_id)
select 'dddd4800-0000-0000-0000-000000000001', c.id
from capabilities c
where c.key = 'projects:finish'
on conflict do nothing;

do $$
declare
  v_tenant_a uuid := 'aaaa4800-0000-0000-0000-000000000001';
  v_admin_id uuid := 'cccc4800-0000-0000-0000-000000000001';
  v_operator_id uuid := 'cccc4800-0000-0000-0000-000000000003';
  v_client_id uuid := 'abab4800-0000-0000-0000-000000000001';
  v_fuel_id uuid := 'eeee4800-0000-0000-0000-000000000001';
  v_project_id uuid := 'tt18-0000-0000-0000-000000000001';
  v_subproject_id uuid := 'tt18-0000-0000-0000-000000000002';
  v_machine_1 uuid := 'tt18-0000-0000-0000-000000000010';
  v_machine_2 uuid := 'tt18-0000-0000-0000-000000000020';
  v_machine_3 uuid := 'tt18-0000-0000-0000-000000000030';
  v_closed int;
  v_count int;
begin
  -- Seed: project
  insert into public.proyectos (id, tenant_id, nombre, cliente_id, ubicacion, fecha_inicio, forma_cobro, monto_fijo, estado)
  values (v_project_id, v_tenant_a, 'Test 18 Project', v_client_id, 'Location 18', '2026-01-01', 'monto_fijo', 5000, 'activo');

  -- Seed: subproject
  insert into public.subproyectos (id, tenant_id, proyecto_id, nombre, ubicacion, estado)
  values (v_subproject_id, v_tenant_a, v_project_id, 'Test 18 SP', 'SP Location', 'activo');

  -- Seed: 3 machines
  insert into public.maquinas (id, tenant_id, codigo, tipo, tipo_combustible_id, tamanio_tanque, modo_medicion_combustible, estado)
  values
    (v_machine_1, v_tenant_a, 'MAQ-T18-D1', 'por_tiempo', v_fuel_id, 100, 'sin_medicion', 'activa'),
    (v_machine_2, v_tenant_a, 'MAQ-T18-D2', 'por_tiempo', v_fuel_id, 100, 'sin_medicion', 'activa'),
    (v_machine_3, v_tenant_a, 'MAQ-T18-SP1', 'por_tiempo', v_fuel_id, 100, 'sin_medicion', 'activa');

  -- Seed: 2 direct assignments on project
  perform public.create_asignacion(v_admin_id, 'test-suite', v_tenant_a, v_machine_1, v_project_id, v_operator_id, 100, null);
  perform public.create_asignacion(v_admin_id, 'test-suite', v_tenant_a, v_machine_2, v_project_id, v_operator_id, 120, null);

  -- Seed: 1 subproject assignment
  perform public.create_asignacion(v_admin_id, 'test-suite', v_tenant_a, v_machine_3, v_project_id, v_operator_id, 150, v_subproject_id);

  -- Act
  v_closed := public.finish_proyecto(v_admin_id, 'test-suite', v_tenant_a, v_project_id, false, null, true);

  -- Assert return value
  if v_closed <> 3 then
    raise exception 'FAIL: expected 3 closed assignments, got %', v_closed;
  end if;

  -- Assert all 3 are cerrada_por_finalizacion
  select count(*) into v_count
  from public.asignaciones_maquina
  where tenant_id = v_tenant_a
    and (proyecto_id = v_project_id or subproyecto_id = v_subproject_id)
    and estado = 'cerrada_por_finalizacion';

  if v_count <> 3 then
    raise exception 'FAIL: expected 3 assignments in cerrada_por_finalizacion, got %', v_count;
  end if;

  raise notice 'PASS: finish_proyecto closed 3 assignments across project and subproject';
end $$;

rollback to savepoint test18;

--
-- TEST 19: Finish subproject — sibling isolation
-- Issue #65 — verify finish_subproyecto only closes its own assignments.
--
\echo 'Test 19: finish_subproyecto sibling isolation'
savepoint test19;
set local role service_role;

-- Grant subprojects:finish to admin role
insert into role_capabilities (role_id, capability_id)
select 'dddd4800-0000-0000-0000-000000000001', c.id
from capabilities c
where c.key = 'subprojects:finish'
on conflict do nothing;
...
declare
  ...
  v_client_id uuid := 'abab4800-0000-0000-0000-000000000001';
  v_fuel_id uuid := 'eeee4800-0000-0000-0000-000000000001';
  v_project_id uuid := 'tt19-0000-0000-0000-000000000001';
  v_sp_a uuid := 'tt19-0000-0000-0000-000000100001';
  v_sp_b uuid := 'tt19-0000-0000-0000-000000200001';
  v_machine_a uuid := 'tt19-0000-0000-0000-000000000010';
  v_machine_b uuid := 'tt19-0000-0000-0000-000000000020';
  v_estado_a text;
  v_estado_b text;
  v_closed int;
begin
  -- Seed: project
  insert into public.proyectos (id, tenant_id, nombre, cliente_id, ubicacion, fecha_inicio, forma_cobro, monto_fijo, estado)
  values (v_project_id, v_tenant_a, 'Test 19 Project', v_client_id, 'Location 19', '2026-01-01', 'monto_fijo', 5000, 'activo');

  -- Seed: 2 subprojects
  insert into public.subproyectos (id, tenant_id, proyecto_id, nombre, ubicacion, estado)
  values
    (v_sp_a, v_tenant_a, v_project_id, 'SP-A', 'Loc A', 'activo'),
    (v_sp_b, v_tenant_a, v_project_id, 'SP-B', 'Loc B', 'activo');

  -- Seed: 2 machines
  insert into public.maquinas (id, tenant_id, codigo, tipo, tipo_combustible_id, tamanio_tanque, modo_medicion_combustible, estado)
  values
    (v_machine_a, v_tenant_a, 'MAQ-T19-A', 'por_tiempo', v_fuel_id, 100, 'sin_medicion', 'activa'),
    (v_machine_b, v_tenant_a, 'MAQ-T19-B', 'por_tiempo', v_fuel_id, 100, 'sin_medicion', 'activa');

  -- Seed: 1 assignment per subproject
  perform public.create_asignacion(v_admin_id, 'test-suite', v_tenant_a, v_machine_a, v_project_id, v_operator_id, 100, v_sp_a);
  perform public.create_asignacion(v_admin_id, 'test-suite', v_tenant_a, v_machine_b, v_project_id, v_operator_id, 120, v_sp_b);

  -- Act: finish subproject A only
  v_closed := public.finish_subproyecto(v_admin_id, 'test-suite', v_tenant_a, v_sp_a, false, null, true);

  if v_closed <> 1 then
    raise exception 'FAIL: expected 1 closed assignment for SP-A, got %', v_closed;
  end if;

  -- Assert: SP-A assignment is cerrada_por_finalizacion
  select estado into v_estado_a
  from public.asignaciones_maquina
  where tenant_id = v_tenant_a and subproyecto_id = v_sp_a;

  if v_estado_a <> 'cerrada_por_finalizacion' then
    raise exception 'FAIL: SP-A assignment expected cerrada_por_finalizacion, got %', v_estado_a;
  end if;

  -- Assert: SP-B assignment is still activa
  select estado into v_estado_b
  from public.asignaciones_maquina
  where tenant_id = v_tenant_a and subproyecto_id = v_sp_b;

  if v_estado_b <> 'activa' then
    raise exception 'FAIL: SP-B assignment expected activa, got %', v_estado_b;
  end if;

  raise notice 'PASS: sibling isolation — SP-A closed, SP-B still active';
end $$;

rollback to savepoint test19;

--
-- TEST 20: p_close_assignments = false
-- Issue #65 — verify opt-out flag prevents assignment closure.
--
\echo 'Test 20: p_close_assignments = false'
savepoint test20;
set local role service_role;
...
declare
  ...
  v_client_id uuid := 'abab4800-0000-0000-0000-000000000001';
  v_fuel_id uuid := 'eeee4800-0000-0000-0000-000000000001';
  v_project_id uuid := 'tt20-0000-0000-0000-000000000001';
  v_machine_id uuid := 'tt20-0000-0000-0000-000000000010';
  v_closed int;
  v_estado text;
  v_proj_estado text;
begin
  -- Seed: project
  insert into public.proyectos (id, tenant_id, nombre, cliente_id, ubicacion, fecha_inicio, forma_cobro, monto_fijo, estado)
  values (v_project_id, v_tenant_a, 'Test 20 Project', v_client_id, 'Location 20', '2026-01-01', 'monto_fijo', 5000, 'activo');

  -- Seed: machine + assignment
  insert into public.maquinas (id, tenant_id, codigo, tipo, tipo_combustible_id, tamanio_tanque, modo_medicion_combustible, estado)
  values (v_machine_id, v_tenant_a, 'MAQ-T20', 'por_tiempo', v_fuel_id, 100, 'sin_medicion', 'activa');

  perform public.create_asignacion(v_admin_id, 'test-suite', v_tenant_a, v_machine_id, v_project_id, v_operator_id, 100, null);

  -- Act: finish with flag disabled
  v_closed := public.finish_proyecto(v_admin_id, 'test-suite', v_tenant_a, v_project_id, false, null, false);

  -- Assert returns 0
  if v_closed <> 0 then
    raise exception 'FAIL: expected 0 closed assignments, got %', v_closed;
  end if;

  -- Assert assignment is still activa
  select estado into v_estado
  from public.asignaciones_maquina
  where tenant_id = v_tenant_a and proyecto_id = v_project_id;

  if v_estado <> 'activa' then
    raise exception 'FAIL: assignment expected activa, got %', v_estado;
  end if;

  -- Assert project was finished
  select estado into v_proj_estado
  from public.proyectos
  where id = v_project_id;

  if v_proj_estado <> 'finalizado' then
    raise exception 'FAIL: project expected finalizado, got %', v_proj_estado;
  end if;

  raise notice 'PASS: p_close_assignments = false — project finished, assignment stays active';
end $$;

rollback to savepoint test20;

--
-- TEST 21: Only activa rows affected
-- Issue #65 — verify closed rows are not touched during finish.
--
\echo 'Test 21: only activa rows affected'
savepoint test21;
set local role service_role;
...
declare
  ...
  v_client_id uuid := 'abab4800-0000-0000-0000-000000000001';
  v_fuel_id uuid := 'eeee4800-0000-0000-0000-000000000001';
  v_project_id uuid := 'tt21-0000-0000-0000-000000000001';
  v_sp_id uuid := 'tt21-0000-0000-0000-000000100001';
  v_machine_1 uuid := 'tt21-0000-0000-0000-000000000010';
  v_machine_2 uuid := 'tt21-0000-0000-0000-000000000020';
  v_machine_3 uuid := 'tt21-0000-0000-0000-000000000030';
  v_assignment_1 uuid;
  v_assignment_2 uuid;
  v_assignment_3 uuid;
  v_count int;
  ...
  v_estado_2 text;
begin
  -- Seed: project + subproject
  insert into public.proyectos (id, tenant_id, nombre, cliente_id, ubicacion, fecha_inicio, forma_cobro, monto_fijo, estado)
  values (v_project_id, v_tenant_a, 'Test 21 Project', v_client_id, 'Location 21', '2026-01-01', 'monto_fijo', 5000, 'activo');

  insert into public.subproyectos (id, tenant_id, proyecto_id, nombre, ubicacion, estado)
  values (v_sp_id, v_tenant_a, v_project_id, 'Test 21 SP', 'SP Location', 'activo');

  -- Seed: 3 machines
  insert into public.maquinas (id, tenant_id, codigo, tipo, tipo_combustible_id, tamanio_tanque, modo_medicion_combustible, estado)
  values
    (v_machine_1, v_tenant_a, 'MAQ-T21-A', 'por_tiempo', v_fuel_id, 100, 'sin_medicion', 'activa'),
    (v_machine_2, v_tenant_a, 'MAQ-T21-B', 'por_tiempo', v_fuel_id, 100, 'sin_medicion', 'activa'),
    (v_machine_3, v_tenant_a, 'MAQ-T21-C', 'por_tiempo', v_fuel_id, 100, 'sin_medicion', 'activa');

  -- Seed: 3 assignments on the subproject
  v_assignment_1 := public.create_asignacion(v_admin_id, 'test-suite', v_tenant_a, v_machine_1, v_project_id, v_operator_id, 100, v_sp_id);
  v_assignment_2 := public.create_asignacion(v_admin_id, 'test-suite', v_tenant_a, v_machine_2, v_project_id, v_operator_id, 120, v_sp_id);
  v_assignment_3 := public.create_asignacion(v_admin_id, 'test-suite', v_tenant_a, v_machine_3, v_project_id, v_operator_id, 150, v_sp_id);

  -- Close 2 assignments manually (to retirada_del_proyecto)
  perform public.update_asignacion(v_admin_id, 'test-suite', v_assignment_2, 'retirada_del_proyecto');
  perform public.update_asignacion(v_admin_id, 'test-suite', v_assignment_3, 'retirada_del_proyecto');

  -- Act: finish subproject (closes only active assignments)
  perform public.finish_subproyecto(v_admin_id, 'test-suite', v_tenant_a, v_sp_id, false, null, true);

  -- Assert: assignment 1 (was active) is now cerrada_por_finalizacion
  select estado into v_estado_1
  from public.asignaciones_maquina where id = v_assignment_1;
  if v_estado_1 <> 'cerrada_por_finalizacion' then
    raise exception 'FAIL: active assignment expected cerrada_por_finalizacion, got %', v_estado_1;
  end if;

  -- Assert: assignment 2 (was retirada) remains retirada_del_proyecto
  select estado into v_estado_2
  from public.asignaciones_maquina where id = v_assignment_2;
  if v_estado_2 <> 'retirada_del_proyecto' then
    raise exception 'FAIL: closed assignment expected retirada_del_proyecto, got %', v_estado_2;
  end if;

  -- Assert: total cerrada_por_finalizacion count is exactly 1
  select count(*) into v_count
  from public.asignaciones_maquina
  where tenant_id = v_tenant_a and estado = 'cerrada_por_finalizacion';

  if v_count <> 1 then
    raise exception 'FAIL: expected 1 cerrada_por_finalizacion, got %', v_count;
  end if;

  raise notice 'PASS: only active assignment transitioned — closed rows untouched';
end $$;

rollback to savepoint test21;

--
-- TEST 22: Audit trail — source = system on auto-close
-- Issue #65 — verify audit_log records system-triggered assignment closure.
--
\echo 'Test 22: audit trail source = system'
savepoint test22;
set local role service_role;
...
declare
  ...
  v_client_id uuid := 'abab4800-0000-0000-0000-000000000001';
  v_fuel_id uuid := 'eeee4800-0000-0000-0000-000000000001';
  v_project_id uuid := 'tt22-0000-0000-0000-000000000001';
  v_machine_id uuid := 'tt22-0000-0000-0000-000000000010';
  v_source text;
  v_action text;
  v_old_estado text;
  v_new_estado text;
begin
  -- Seed: project
  insert into public.proyectos (id, tenant_id, nombre, cliente_id, ubicacion, fecha_inicio, forma_cobro, monto_fijo, estado)
  values (v_project_id, v_tenant_a, 'Test 22 Project', v_client_id, 'Location 22', '2026-01-01', 'monto_fijo', 5000, 'activo');

  -- Seed: machine + assignment
  insert into public.maquinas (id, tenant_id, codigo, tipo, tipo_combustible_id, tamanio_tanque, modo_medicion_combustible, estado)
  values (v_machine_id, v_tenant_a, 'MAQ-T22', 'por_tiempo', v_fuel_id, 100, 'sin_medicion', 'activa');

  perform public.create_asignacion(v_admin_id, 'test-suite', v_tenant_a, v_machine_id, v_project_id, v_operator_id, 100, null);

  -- Act: finish project
  perform public.finish_proyecto(v_admin_id, 'test-suite', v_tenant_a, v_project_id, false, null, true);

  -- Assert: audit_log has system source for assignment update
  select source, action, old_value->>'estado', new_value->>'estado'
    into v_source, v_action, v_old_estado, v_new_estado
  from public.audit_log
  where tenant_id = v_tenant_a
    and action = 'asignacion.update'
    and source = 'system'
  ...
  if v_source is null or v_source <> 'system' then
    raise exception 'FAIL: expected source = system, got %', v_source;
  end if;

  if v_action <> 'asignacion.update' then
    raise exception 'FAIL: expected action = asignacion.update, got %', v_action;
  end if;

  if v_old_estado <> 'activa' then
    raise exception 'FAIL: expected old estado activa, got %', v_old_estado;
  end if;

  if v_new_estado <> 'cerrada_por_finalizacion' then
    raise exception 'FAIL: expected new estado cerrada_por_finalizacion, got %', v_new_estado;
  end if;

  raise notice 'PASS: audit trail records source=system, action=asignacion.update, activa→cerrada_por_finalizacion';
end $$;
