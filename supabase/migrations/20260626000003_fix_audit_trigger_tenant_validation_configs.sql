-- Ensure audit_trigger() derives tenant_id from tenant_validation_configs rows.
-- This avoids audit_log NOT NULL failures for service/admin/test writes that do
-- not run with request JWT tenant context.
...
begin
  if TG_TABLE_NAME = 'tenants' then
    ...
    _tenant_id := coalesce(NEW.tenant_id, OLD.tenant_id);
  elsif TG_TABLE_NAME = 'tenant_validation_configs' then
    _tenant_id := coalesce(NEW.tenant_id, OLD.tenant_id);
  else
